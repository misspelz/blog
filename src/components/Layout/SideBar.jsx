...
const SideBar = () => {
  ...
  return (
    <div className="flex flex-col w-[30%] hidden md:block hidden md:block  bg-white p-4 px-6">
      ...
      <div className="mt-8 font-semibold text-xl text-[#023047]">Other Posts </div>

      <div className="mb-8">
       {fromLocalForBlog && fromLocalForBlog.map((item)=>{
        return (
          <div key={item.id}>
            <h3 className="mt-4 hover:text-[#ffa500] cursor-pointer">{item.title}</h3>
          </div>
        )
       })}
      </div>
